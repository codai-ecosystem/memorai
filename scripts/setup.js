...
class MemoraiSetup {
  ...
  async checkOpenAI() {
    ...
    if (!apiKey) {
      ...
      console.log(
        '💡 Set MEMORAI_OPENAI_API_KEY or OPENAI_API_KEY for Tier 1 (Advanced Memory)'
      );
      return;
    }
  ...
  async checkLocalAI() {
    ...
    try {
      ...
      const sentenceTransformersAvailable =
  ...
  async generateReport() {
    ...
    const config = {
      ...
      tiers: this.results,
      recommended: this.results.openai
        ? 'advanced'
        : this.results.localAI
          ? 'smart'
          : 'basic',
    };
