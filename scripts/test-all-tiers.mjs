 ...
 * Tests all 4 memory tiers and validates functionality
 */

import { UnifiedMemoryEngine, MemoryTierLevel } from '../packages/core/dist/index.js';
...
class MemoraiTester {
    ...
    async runAllTests() {
        ...
        await this.testTier(MemoryTierLevel.MOCK);
    ...
    async runBasicTests(engine, tier) {
        ...
        try {
            ...
            console.log(`      ✅ Recall: ${tests.recall.time}ms (${results.length} results)`);
            ...
            console.log(`      ✅ Context: ${tests.context.time}ms (${context.memories.length} memories)`);
            ...
            console.log(`      ✅ Forget: ${tests.forget.time}ms (${forgotten ? 'success' : 'failed'})`);
    ...
    async testFallbackSystem() {
        ...
        try {
            ...
            await engine.remember('Fallback test memory', 'test-tenant', 'test-agent');
    ...
    async generateReport() {
        ...
        for (const [tier, result] of Object.entries(this.results.tiers)) {
            ...
            console.log(`   ${status} ${tier.toUpperCase()}: ${result.available ? `${result.initTime}ms init` : result.error}`);
        }
        ...
        for (const [tier, result] of Object.entries(this.results.tiers)) {
            if (result.available && result.tests) {
                ...
                if (result.tests.remember) console.log(`     Remember: ${result.tests.remember.time}ms`);
                if (result.tests.recall) console.log(`     Recall: ${result.tests.recall.time}ms`);
                if (result.tests.context) console.log(`     Context: ${result.tests.context.time}ms`);
                if (result.tests.forget) console.log(`     Forget: ${result.tests.forget.time}ms`);
            }
        }
        ...
        if (this.results.tests.autoDetection) {
            ...
            console.log(`   ${status} Auto-Detection: ${test.success ? test.detectedTier : test.error}`);
        }

        if (this.results.tests.fallback) {
            const test = this.results.tests.fallback;
            const status = test.success ? '✅' : '❌';
            console.log(`   ${status} Fallback System: ${test.success ? test.activeTier : test.error}`);
        }
        ...
        if (availableTiers.includes('advanced')) {
            ...
            console.log('   🧪 Only Mock tier available (development mode)');
        }
        ...
        const reportData = {
            ...
            summary: {
                ...
                totalTiers: 4,
                recommendedTier: availableTiers.includes('advanced') ? 'advanced' :
                    availableTiers.includes('smart') ? 'smart' :
                        availableTiers.includes('basic') ? 'basic' : 'mock'
            },
            results: this.results
        };

        const fs = await import('fs');
        fs.writeFileSync('memorai-test-report.json', JSON.stringify(reportData, null, 2));
