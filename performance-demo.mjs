...
class MemoraiPerformanceDemo {
  ...
  async measureOperation(name, operation) {
    ...
    try {
      ...
      this.log(
        `❌ ${name}: ${responseTime.toFixed(2)}ms - ${error.message}`,
        'ERROR'
      );
      throw error;
    }
  ...
  generatePerformanceReport() {
    ...
    const avgResponseTime =
      this.results.reduce((sum, r) => sum + parseFloat(r.responseTime), 0) /
      this.results.length;
    const successRate =
      (this.results.filter(r => r.success).length / this.results.length) * 100;
    const fastOperations = this.results.filter(
      r => parseFloat(r.responseTime) < 50
    ).length;
    ...
    this.log(
      `Fast Operations (<50ms): ${fastOperations}/${this.results.length}`,
      'REPORT'
    );
    this.log(
      `Operations/Second: ${((this.operations / totalTime) * 1000).toFixed(2)}`,
      'REPORT'
    );
    this.log('=' * 50, 'REPORT');
    ...
    const benchmarks = {
      'Sub-10ms Operations': this.results.filter(
        r => parseFloat(r.responseTime) < 10
      ).length,
      'Sub-25ms Operations': this.results.filter(
        r => parseFloat(r.responseTime) < 25
      ).length,
      'Sub-50ms Operations': fastOperations,
      'Enterprise Grade (>95% success)':
        successRate > 95 ? '✅ PASSED' : '❌ FAILED',
    };
