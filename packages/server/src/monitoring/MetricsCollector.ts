...
export class MetricsCollector {
    ...
    public getServerMetrics(): ServerMetrics {
        ...
        const requestsPerSecond = (recentRequests.length * 1000) / this.windowSizeMs;

        // Calculate average response time
        const averageResponseTime = this.requests.count > 0
    ...
    public getDetailedMetrics(): any {
        ...
        return {
            ...
            requests: {
                ...
                successRate: this.requests.count > 0
                    ? ((this.requests.count - this.requests.errors) / this.requests.count) * 100
                    : 100
            },
            history: {
     ...
     */
    private addDataPoint(history: MetricDataPoint[], timestamp: number, value: number): void {
