...
describe('MemoraiClient - Coverage Enhancement', () => {
  ...
  describe('getStats error handling', () => {
    it('should handle getStats error response (lines 302-303)', async () => {
      ...
      await expect(client.getStats()).rejects.toThrow('Failed to get stats: Internal error occurred while getting stats');
    ...
    it('should handle getStats with different error message format', async () => {
      ...
      await expect(client.getStats()).rejects.toThrow('Failed to get stats: Invalid params: missing required fields');
    });
  });
  describe('clearSession functionality', () => {
    it('should handle cache clear during clearSession when cache enabled (line 277-278)', async () => {
      ...
      const clientOptionsWithCache: ClientOptions & { agentId: string; sessionId?: string; tenantId?: string } = {
        serverUrl: 'http://localhost:3000',
        agentId: 'test-agent',
        sessionId: 'test-session',
        tenantId: 'test-tenant',
      ...
      await clientWithCache.clearSession();      expect(mockConnection.send).toHaveBeenCalledWith({
        jsonrpc: '2.0',
    ...
    it('should handle clearSession when cache disabled', async () => {
      ...
      const clientOptionsNoCache: ClientOptions & { agentId: string; sessionId?: string; tenantId?: string } = {
        serverUrl: 'http://localhost:3000',
        agentId: 'test-agent',
        sessionId: 'test-session',
        tenantId: 'test-tenant',
