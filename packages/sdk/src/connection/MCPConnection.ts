import type { RecallOptions, AgentMemory, ContextOptions, ConnectionOptions } from '../types/index.js';
...
export class MCPConnection {
  ...
  private pendingRequests = new Map<string | number, { resolve: (value: any) => void; reject: (error: any) => void; timeout?: NodeJS.Timeout }>();
  ...
  private async makeRequest(method: string, endpoint: string, body?: unknown): Promise<Response> {
    ...
    const config: RequestInit = {
      ...
      ...(this.options.timeout && { signal: AbortSignal.timeout(this.options.timeout) })
    };
  ...
  async remember(agentId: string, content: string, metadata?: Record<string, unknown>): Promise<void> {
    if (!this.connected) {
  ...
  async recall(agentId: string, query: string, options?: RecallOptions): Promise<AgentMemory[]> {
    if (!this.connected) {
      throw new Error('MCP connection not established');
    }
  ...
  async getContext(agentId: string, options?: ContextOptions): Promise<AgentMemory[]> {
    if (!this.connected) {
      throw new Error('MCP connection not established');
    }
  ...
  async forget(agentId: string, memoryId: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to forget memory: ${error instanceof Error ? error.message : String(error)}`);
    }
  }  async send(request: {
    jsonrpc: string;
    method: string;
