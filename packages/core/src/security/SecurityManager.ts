...
export class InputValidator {
   ...
   */
  public static validate(input: Record<string, unknown>, rules: ValidationRule[]): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    for (const rule of rules) {
      ...
      if (rule.customValidator) {
        ...
        if (customResult !== true) {
          errors.push(typeof customResult === 'string' ? customResult : `Field '${rule.field}' is invalid`);
        }
      }
    }
...
export class RateLimiter {
   ...
   */
  public isAllowed(tenantId: string, agentId?: string): {
    allowed: boolean;
   ...
   */
  public getStatus(tenantId: string, agentId?: string): {
    requestCount: number;
    remainingRequests: number;
    resetTime: Date;
  } {
...
export class SecurityManager {
   ...
   * Validate input data
   */
  public validateInput(input: Record<string, unknown>, rules: ValidationRule[]): {
    isValid: boolean;
    errors: string[];
  } {
  ...
  public getAuditEvents(filter?: {
    ...
    if (filter.eventType) {
      auditOptions.eventType = filter.eventType as SecurityAuditEvent['eventType'];
    }
    if (filter.severity) {
