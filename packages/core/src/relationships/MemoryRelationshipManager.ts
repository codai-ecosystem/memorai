...
export class MemoryRelationshipManager implements IMemoryRelationshipManager {
    ...
    async getRelationships(memoryId: string, tenantId: string): Promise<MemoryRelationship[]> {
        const key = `${memoryId}_${tenantId}`;
        return this.relationships.get(key) || [];
    }

    async deleteRelationship(relationshipId: string, tenantId: string): Promise<boolean> {
    ...
    async suggestRelationships(memoryId: string, tenantId: string): Promise<MemoryRelationship[]> {
     ...
     */
    async getMemoryHierarchy(memoryId: string, tenantId: string): Promise<{
        parents: string[];
        children: string[];
        siblings: string[];
    }> {
     ...
     */
    async cleanupOrphanedRelationships(validMemoryIds: string[], tenantId: string): Promise<number> {
        let cleanupCount = 0;
        ...
        logger.info('Cleaned up orphaned relationships', { count: cleanupCount, tenantId });
        return cleanupCount;
    }
}
