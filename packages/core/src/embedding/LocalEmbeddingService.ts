...
export class LocalEmbeddingService {
    ...
    public async embed(text: string): Promise<EmbeddingResult> {
        ...
        try {
            ...
            throw new Error(`Local embedding generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
            ...
            python.on('close', (code: number | null) => {
                if (code === 0) {
                    try {
                        ...
                        reject(new Error(`Failed to parse embedding result: ${error instanceof Error ? error.message : 'Unknown error'}`));
                    }
                } else {
...
`;
   ...
   */  public static async isAvailable(pythonPath = 'python'): Promise<boolean> {
        return new Promise((resolve) => {
            const python: ChildProcess = spawn(pythonPath, ['-c', 'import sentence_transformers; print("OK")']);
    ...
    public static async install(pythonPath = 'python'): Promise<boolean> {
        return new Promise((resolve) => {
            const pip: ChildProcess = spawn(pythonPath, ['-m', 'pip', 'install', 'sentence-transformers']);
