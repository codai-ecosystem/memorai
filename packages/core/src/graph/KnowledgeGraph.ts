...
export class KnowledgeGraph {
   ...
   */  async addEntity(
    ...
    if (existingEntity) {
      ...
      existingEntity.properties = { ...existingEntity.properties, ...properties };
  ...
  findRelations(query: GraphQuery): GraphRelation[] {
    let results = Array.from(this.relations.values())
      .filter(r => r.tenant_id === query.tenant_id && (!query.agent_id || r.agent_id === query.agent_id));
    ...
    const visited = new Set<string>();    const dfs = (currentId: string, targetId: string, currentPath: string[], currentRelations: string[], currentWeight: number, depth: number) => {
      ...
      if (currentId === targetId && currentPath.length > 1) {
        ...
        const confidence = relations.length > 0 ?
          relations.reduce((sum, r) => sum + r.confidence, 0) / relations.length : 1.0;
   ...
   */
  findShortestPath(fromId: string, toId: string, maxDepth: number = 5): GraphPath | null {
    if (!this.entities.has(fromId) || !this.entities.has(toId)) {
      return null;
    }

    const visited = new Set<string>();
    const queue: Array<{ entityId: string; path: string[]; relations: string[]; totalWeight: number }> = [];

    queue.push({ entityId: fromId, path: [fromId], relations: [], totalWeight: 0 });
    visited.add(fromId);
    ...
    while (queue.length > 0) {
      ...
      for (const relationId of relationIds) {
        const relation = this.relations.get(relationId)!;
        const neighborId = relation.fromId === current.entityId ? relation.toId : relation.fromId;
  ...
  getAnalytics(tenant_id: string, agent_id?: string): GraphAnalytics {
    ...
    const entities = Array.from(this.entities.values())
      .filter(e => e.tenant_id === tenant_id && (!agent_id || e.agent_id === agent_id));

    const relations = Array.from(this.relations.values())
      .filter(r => r.tenant_id === tenant_id && (!agent_id || r.agent_id === agent_id));    // Calculate metrics
    ...
    entities.forEach(entity => {
      const connectionCount = (this.entityRelations.get(entity.id) || new Set()).size;
   ...
   * Export graph data
   */
  exportGraph(tenant_id: string, agent_id?: string): { entities: GraphEntity[]; relations: GraphRelation[] } {
    const entities = Array.from(this.entities.values())
      .filter(e => e.tenant_id === tenant_id && (!agent_id || e.agent_id === agent_id));

    const relations = Array.from(this.relations.values())
      .filter(r => r.tenant_id === tenant_id && (!agent_id || r.agent_id === agent_id));
   ...
   * Import graph data
   */
  async importGraph(data: { entities: GraphEntity[]; relations: GraphRelation[] }): Promise<void> {
    // Import entities
  ...
  private findEntityByNameAndType(name: string, type: string, tenant_id: string): GraphEntity | undefined {
    return Array.from(this.entities.values()).find(
      e => e.name === name && e.type === type && e.tenant_id === tenant_id
    );
  }

  private findExistingRelation(fromId: string, toId: string, type: string, tenant_id: string): string | undefined {
