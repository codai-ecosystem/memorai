  ...
  private async gradualTrafficShift(
    ...
    for (const percentage of steps) {
      ...
      if (metrics.errorRate > 0.05) {
        // 5% error threshold
        throw new Error(
          `High error rate detected: ${metrics.errorRate * 100}%`
        );
      }
...
export class InfrastructureAsCodeManager {
  ...
  getDeployedResources(): Array<{
    id: string;
    templateName: string;
    deployedAt: Date;
  }> {
    return Array.from(this.deployedResources.entries()).map(
      ([id, deployment]) => ({
        id,
...
export class DeploymentOrchestrationEngine extends EventEmitter {
  ...
  async setupInfrastructure(
    ...
    try {
      const deploymentId = await this.iacManager.deployInfrastructure(
        templateName,
        variables
      );
  ...
  private async performRecreateDeployment(
    version: string
  ): Promise<DeploymentStatus> {
    const deploymentId = uuidv4();
