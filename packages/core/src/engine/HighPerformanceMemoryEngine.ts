...
import { HighPerformanceCache, memoryCache } from '../cache/HighPerformanceCache.js';
...
export class HighPerformanceMemoryEngine {
    ...
    public async recall(
        ...
        try {
            ...
            if (options.useCache !== false) {
                const cachedResults = this.cache.getCachedMemoryResults(query, tenantId, agentId, options);
                if (cachedResults) {
    ...
    private classifyMemoryType(content: string): MemoryType {
        ...
        if (lower.includes('feel') || lower.includes('emotion') || lower.includes('happy') || lower.includes('sad')) {
            return 'emotion';
        }
        if (lower.includes('task') || lower.includes('todo') || lower.includes('complete') || lower.includes('finish')) {
            return 'task';
        }
    ...
    private calculateImportance(content: string): number {
        ...
        const priorityWords = ['important', 'critical', 'urgent', 'remember', 'key', 'essential'];
        const lower = content.toLowerCase();
