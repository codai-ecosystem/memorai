...
export class MemoryConfigManager {
  ...
  private mergeConfig(
    ...
    const safeOverrides = overrides || {};
    return {
      vector_db: { ...defaultConfig.vector_db, ...(safeOverrides.vector_db || {}) },
      redis: { ...defaultConfig.redis, ...(safeOverrides.redis || {}) },
      embedding: { ...defaultConfig.embedding, ...(safeOverrides.embedding || {}) },
    ...
    if (!this.config.embedding.model || this.config.embedding.model.trim().length === 0) {
      throw new Error('Invalid memory configuration: Embedding model cannot be empty');
    }
    if (this.config.embedding.api_key && this.config.embedding.api_key.length < 10) {
    ...
    try {
      ...
      import('../types/index.js').then(({ MemoryConfigSchema }) => {
        ...
        if (error instanceof z.ZodError) {
          ...
          console.error(`Memory configuration validation warning: ${JSON.stringify(error.errors, null, 2)}`);
        }
      });
    } catch (error: unknown) {
      if (error instanceof z.ZodError) {
