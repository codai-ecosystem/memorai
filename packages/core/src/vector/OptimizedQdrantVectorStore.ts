...
export class OptimizedQdrantVectorStore implements VectorStore {
    ...
    private getConnection(): QdrantClient {
        ...
        this.currentConnectionIndex = (this.currentConnectionIndex + 1) % this.config.connectionPoolSize;
        return connection;
    }
    ...
    public async checkDuplicateByHash(contentHash: string, tenantId: string, agentId?: string): Promise<string | null> {
        const connection = this.getConnection();
        ...
        return searchResult.points.length > 0 ? searchResult.points[0].id as string : null;
    }
