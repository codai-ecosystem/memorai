...
export class GitHubIntegration {
  ...
  async extractPullRequestContext(): Promise<PullRequestContext[]> {
    ...
    try {
      ...
      for (const pull of pulls) {
        try {
          const context = await this.extractSinglePullRequestContext(
            pull.number
          );
          contexts.push(context);
        } catch (error) {
      ...
      if ('content' in fileData && fileData.content) {
        ...
        return {
          ...
          lastModified: new Date(
            lastCommit?.commit.committer?.date || Date.now()
          ),
  ...
  private async extractSingleIssueContext(
    issueNumber: number
  ...
  private async extractSinglePullRequestContext(
    prNumber: number
    ...
    try {
      switch (language) {
        ...
        case 'typescript':
          ...
          const jsPattern =
            /(?:function\s+(\w+)|(\w+)\s*(?::\s*\w+)?\s*=\s*(?:async\s+)?(?:\([^)]*\)\s*=>|\([^)]*\)\s*{)|(\w+)\s*\([^)]*\)\s*{)/g;
          let jsMatch;
