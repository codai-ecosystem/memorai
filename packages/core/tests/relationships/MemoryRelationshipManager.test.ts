...
describe('MemoryRelationshipManager', () => {
  ...
  describe('deleteRelationship', () => {
    it('should delete an existing relationship', async () => {
      ...
      const relationship = await manager.createRelationship(relationshipData);
      const deleted = await manager.deleteRelationship(
        relationship.id,
        tenantId
      );
  ...
  describe('buildMemoryGraph', () => {
    ...
    it('should filter by relationship types', async () => {
      ...
      const parentRelationships = result.edges.filter(
        e => e.relationshipType === 'parent'
      );
  ...
  describe('getMemoryHierarchy', () => {
    beforeEach(async () => {
      ...
      await manager.createParentChildRelationship(
        'parent-memory',
        memoryId1,
        tenantId
      );
      ...
      await manager.createSiblingRelationship(
        memoryId1,
        'sibling-memory',
        tenantId
      );
  ...
  describe('cleanupOrphanedRelationships', () => {
    it('should remove relationships with invalid memory IDs', async () => {
      ...
      const cleanupCount = await manager.cleanupOrphanedRelationships(
        [memoryId1, memoryId2],
        tenantId
      );
      ...
      const remainingRelationships = await manager.getRelationships(
        memoryId1,
        tenantId
      );
  ...
  describe('suggestRelationships', () => {
    it('should return empty array as placeholder', async () => {
      const suggestions = await manager.suggestRelationships(
        memoryId1,
        tenantId
      );
