...
} from '../../src/integrations/GitHubIntegration.js';
...
describe('GitHubIntegration', () => {
  ...
  describe('extractCodeContext', () => {
    it('should extract code context from repository files', async () => {
      ...
      mockOctokitMethods.rest.repos.getContent.mockResolvedValue({
        data: {
          content: Buffer.from('function hello() { return "world"; }').toString(
            'base64'
          ),
        },
      });
      ...
      expect(contexts).toHaveLength(2); // Only .ts and .js files, excluding node_modules
    ...
    it('should handle API errors gracefully', async () => {
      mockOctokitMethods.rest.git.getTree.mockRejectedValue(
        new Error('API Error')
      );
  ...
  describe('Code Parsing', () => {
    it('should extract JavaScript functions correctly', () => {
      ...
      `;
      const functions = (integration as any).extractFunctions(
        content,
        'javascript'
      );
    ...
    it('should extract classes correctly', () => {
      ...
      `;
      const classes = (integration as any).extractClasses(
        content,
        'javascript'
      );
    ...
    it('should extract imports correctly', () => {
      ...
      `;
      const imports = (integration as any).extractImports(
        content,
        'javascript'
      );
  ...
  describe('File Filtering', () => {
    ...
    it('should filter files by include paths correctly', () => {
      ...
      expect((testIntegration as any).shouldProcessFile('src/test.ts')).toBe(
        true
      );
      expect((testIntegration as any).shouldProcessFile('lib/utils.js')).toBe(
        true
      );
      expect((testIntegration as any).shouldProcessFile('docs/readme.md')).toBe(
        false
      );
    });
    ...
    it('should process all files when no filters are specified', () => {
      ...
      expect(
        (testIntegration as any).shouldProcessFile('node_modules/package.json')
      ).toBe(true);
    });
  });
});
