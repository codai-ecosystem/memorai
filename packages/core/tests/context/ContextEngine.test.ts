...
describe('ContextEngine', () => {
    ...
    mockMemories = [
      ...
      },
      {
        memory: {
          id: 'mem4',
          type: 'task',
          content: 'User is working on a machine learning project deadline next week',
          confidence: 0.95,
          importance: 0.9,
  ...
  describe('generateContextSummary', () => {
    ...
    it('should handle empty memories array', () => {
      ...
      const summary = contextEngine.generateContextSummary(mockMemories, { maxLength: 100 });
    ...
    it('should include scores when requested', () => {
      const summary = contextEngine.generateContextSummary(mockMemories, { includeScore: true });
      expect(summary).toMatch(/\[0\.\d{2}\]/); // Score format [0.XX]
    });    it('should include timestamps when requested', () => {
      const summary = contextEngine.generateContextSummary(mockMemories, { includeTimestamp: true });
  ...
  describe('extractThemes', () => {
    it('should extract and rank themes from memories', () => {
      ...
      for (let i = 1; i < themes.length; i++) {
        const prevRelevance = themes[i - 1].frequency * themes[i - 1].importance;
  ...
  describe('analyzeEmotionalContext', () => {
    it('should analyze emotional context from memories', () => {
      ...
      expect(['positive', 'neutral', 'negative']).toContain(emotional.overall_sentiment);
    ...
    it('should handle memories without emotional weights', () => {
      ...
      const emotional = contextEngine.analyzeEmotionalContext(memoriesNoEmotion);
  ...
  describe('filterContextualMemories', () => {
    it('should filter memories by importance threshold', () => {
      const filtered = contextEngine.filterContextualMemories(mockMemories, 20, 0.7);
    ...
    it('should respect maximum memory limit', () => {
      const maxMemories = 3;
      const filtered = contextEngine.filterContextualMemories(mockMemories, maxMemories);
    ...
    it('should handle threshold above all importances', () => {
      const filtered = contextEngine.filterContextualMemories(mockMemories, 20, 0.99);
  ...
  describe('private method coverage via public methods', () => {
    it('should test groupMemoriesByType via generateContextSummary', () => {
      ...
      const summary = contextEngine.generateContextSummary(memoryWithUnderscore);
    ...
    it('should test calculateContextConfidence via generateAgentContext', () => {
      ...
      const expectedKeywords = ['machine', 'learning', 'algorithm', 'optimization', 'performance'];
      const foundKeywords = expectedKeywords.filter(keyword => themeWords.includes(keyword));
    ...
    it('should test calculateCompositeScore via filterContextualMemories', () => {
      ...
      const filtered = contextEngine.filterContextualMemories(memoriesWithDifferentAges);
  ...
  describe('edge cases and error handling', () => {
    it('should handle memories with missing optional fields', () => {
      ...
      const summary = contextEngine.generateContextSummary(memoryWithLongContent, { maxLength: 100 });
      expect(summary.length).toBeLessThanOrEqual(130); // Should truncate
      expect(summary).toContain('[Context truncated]');
    });

    it('should handle extreme importance threshold', () => {
      const filtered = contextEngine.filterContextualMemories(mockMemories, 20, 1.1); // Above max
      expect(filtered).toEqual([]);

      const filtered2 = contextEngine.filterContextualMemories(mockMemories, 20, -0.1); // Below min
