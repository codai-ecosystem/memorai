...
import { TemporalEngine, DEFAULT_DECAY_PARAMETERS } from '../../src/temporal/TemporalEngine.js';
...
describe('TemporalEngine', () => {
  ...
  beforeEach(() => {
    ...
    currentTime = new Date('2024-01-15T12:00:00Z');    baseMemory = {
      id: 'test-memory',
      agent_id: 'test-agent',
  ...
  describe('constructor', () => {
    it('should initialize with default decay parameters', () => {
      ...
      expect(defaultEngine.getDecayParameters('fact')).toEqual(DEFAULT_DECAY_PARAMETERS.fact);
      expect(defaultEngine.getDecayParameters('personality')).toEqual(DEFAULT_DECAY_PARAMETERS.personality);
      expect(defaultEngine.getDecayParameters('procedure')).toEqual(DEFAULT_DECAY_PARAMETERS.procedure);
    });
    ...
    it('should apply custom decay parameters', () => {
      ...
      expect(factParams.frequency).toBe(DEFAULT_DECAY_PARAMETERS.fact.frequency); // Should keep default
      
      const personalityParams = customEngine.getDecayParameters('personality');
      expect(personalityParams.emotional).toBe(0.8);
      expect(personalityParams.importance).toBe(DEFAULT_DECAY_PARAMETERS.personality.importance); // Should keep default
    });
    ...
    it('should handle partial custom parameters', () => {
      ...
      expect(threadParams.frequency).toBe(DEFAULT_DECAY_PARAMETERS.thread.frequency);
      expect(threadParams.emotional).toBe(DEFAULT_DECAY_PARAMETERS.thread.emotional);
      expect(threadParams.baseDecayRate).toBe(DEFAULT_DECAY_PARAMETERS.thread.baseDecayRate);
    });

    it('should handle empty custom parameters', () => {
      const customEngine = new TemporalEngine({});
      
      expect(customEngine.getDecayParameters('fact')).toEqual(DEFAULT_DECAY_PARAMETERS.fact);
    });
  });

  describe('calculateTimeDecay', () => {    it('should calculate decay for a basic memory', () => {
    ...
    it('should handle different memory types with different decay rates', () => {
      const personalityMemory = { ...baseMemory, type: 'personality' as MemoryType };
      const threadMemory = { ...baseMemory, type: 'thread' as MemoryType };
      
      const personalityDecay = engine.calculateTimeDecay(personalityMemory, currentTime);
    ...
    it('should handle emotional weight in decay calculation', () => {
      ...
      const emotionalDecay = engine.calculateTimeDecay(emotionalMemory, currentTime);
      const normalDecay = engine.calculateTimeDecay(nonEmotionalMemory, currentTime);
    ...
    it('should handle negative emotional weight', () => {
      ...
      const decay = engine.calculateTimeDecay(negativeEmotionalMemory, currentTime);
    ...
    it('should handle high importance memories', () => {
      ...
      const highDecay = engine.calculateTimeDecay(highImportanceMemory, currentTime);
      const lowDecay = engine.calculateTimeDecay(lowImportanceMemory, currentTime);
    ...
    it('should handle frequently accessed memories', () => {
      ...
      const frequentDecay = engine.calculateTimeDecay(frequentMemory, currentTime);
    ...
    it('should handle very old memories', () => {
      const oldMemory = {
        ...
        lastAccessedAt: new Date(currentTime.getTime() - 1000 * 60 * 60 * 24 * 365) // 1 year ago
      };
    ...
    it('should ensure decay is between 0 and 1', () => {
      const extremeMemory = {
        ...
        lastAccessedAt: new Date(currentTime.getTime() - 1000 * 60 * 60 * 24 * 1000)
      };
  ...
  describe('shouldArchive', () => {
    ...
    it('should return false for memories above archive threshold', () => {
      ...
      const shouldArchive = engine.shouldArchive(strongMemory, 0.1, currentTime);
  ...
  describe('shouldForget', () => {
    it('should return true for memories past TTL', () => {
      ...
      const shouldForget = engine.shouldForget(expiredMemory, 0.05, currentTime);
  ...
  describe('calculateTemporalRelevance', () => {
    it('should calculate relevance with temporal factors', () => {
      ...
      const relevance = engine.calculateTemporalRelevance(baseMemory, baseRelevance, currentTime);
    ...
    it('should boost recent memories (created < 24 hours ago)', () => {
      ...
      const relevance = engine.calculateTemporalRelevance(recentMemory, baseRelevance, currentTime);
      const normalRelevance = engine.calculateTemporalRelevance(baseMemory, baseRelevance, currentTime);
    ...
    it('should boost recently accessed memories (< 1 hour ago)', () => {
      ...
      const baseRelevance = 0.5;
      const relevance = engine.calculateTemporalRelevance(recentlyAccessedMemory, baseRelevance, currentTime);
      const normalRelevance = engine.calculateTemporalRelevance(baseMemory, baseRelevance, currentTime);
    ...
    it('should use current time as default', () => {
      ...
      const relevance = engine.calculateTemporalRelevance(baseMemory, baseRelevance);
  ...
  describe('getMemoriesForMaintenance', () => {
    ...
    beforeEach(() => {
      memories = [
        { ...baseMemory, id: 'strong-memory', confidence: 0.9, importance: 0.9 },
        { ...baseMemory, id: 'weak-memory', confidence: 0.05 },
        { ...baseMemory, id: 'expired-memory', ttl: new Date(currentTime.getTime() - 1000) },
        { ...baseMemory, id: 'medium-memory', confidence: 0.08 }
      ];
    });    it('should categorize memories for maintenance', () => {
      const result = engine.getMemoriesForMaintenance(memories, 0.1, 0.05, currentTime);
      ...
      expect(result.toArchive.some(m => m.id === 'strong-memory')).toBe(false);
      expect(result.toForget.some(m => m.id === 'strong-memory')).toBe(false);
      
      // Total should match original memories count
      expect(result.toArchive.length + result.toForget.length).toBeLessThanOrEqual(memories.length);
    });

    it('should handle empty memory list', () => {
      const result = engine.getMemoriesForMaintenance([], 0.1, 0.05, currentTime);
    ...
    it('should prioritize forgetting over archiving', () => {
      const veryWeakMemory = { ...baseMemory, id: 'very-weak', confidence: 0.01 };
      const memoriesWithVeryWeak = [...memories, veryWeakMemory];
      
      const result = engine.getMemoriesForMaintenance(memoriesWithVeryWeak, 0.1, 0.05, currentTime);
  ...
  describe('setDecayParameters', () => {
    it('should update decay parameters for a memory type', () => {
      ...
      expect(updatedParams.frequency).toBe(DEFAULT_DECAY_PARAMETERS.fact.frequency); // Should preserve
    });
    ...
    it('should update all memory types independently', () => {
      ...
      expect(engine.getDecayParameters('personality').importance).toBe(DEFAULT_DECAY_PARAMETERS.personality.importance);
    });
  });
  ...
  describe('getDecayParameters', () => {
    ...
    it('should handle all memory types', () => {
      const types: MemoryType[] = ['personality', 'procedure', 'preference', 'fact', 'thread', 'task', 'emotion'];
  ...
  describe('resetDecayParameters', () => {
    it('should reset all decay parameters to defaults', () => {
      ...
      expect(engine.getDecayParameters('fact')).toEqual(DEFAULT_DECAY_PARAMETERS.fact);
      expect(engine.getDecayParameters('personality')).toEqual(DEFAULT_DECAY_PARAMETERS.personality);
    });
    ...
    it('should reset all memory types', () => {
      ...
      for (const type of types) {
        expect(engine.getDecayParameters(type)).toEqual(DEFAULT_DECAY_PARAMETERS[type]);
      }
    });
  });
  ...
  describe('DEFAULT_DECAY_PARAMETERS', () => {
    it('should have parameters for all memory types', () => {
      ...
      for (const type of types) {
        ...
        expect(typeof DEFAULT_DECAY_PARAMETERS[type].baseDecayRate).toBe('number');
      }
    });
    ...
    it('should have different decay rates for different memory types', () => {
      // Personality should decay slower than thread
      expect(DEFAULT_DECAY_PARAMETERS.personality.baseDecayRate)
        .toBeLessThan(DEFAULT_DECAY_PARAMETERS.thread.baseDecayRate);
      
      // Procedure should decay slower than fact
      expect(DEFAULT_DECAY_PARAMETERS.procedure.baseDecayRate)
        .toBeLessThan(DEFAULT_DECAY_PARAMETERS.fact.baseDecayRate);
    });
  });
});
