...
vi.mock('../../src/embedding/EmbeddingService.js', () => ({
  EmbeddingService: vi.fn().mockImplementation(() => ({
    embed: vi.fn().mockImplementation(async (text: string) => {
      ...
      return {
        embedding: new Array(1536).fill(0).map((_, i) => Math.sin(i * 0.1) * Math.random()),
...
vi.mock('../../src/vector/VectorStore.js', () => ({
  InMemoryVectorStore: vi.fn().mockImplementation(() => ({
    ...
    search: vi.fn().mockImplementation(async (embedding: number[], query: any = {}) => {
...
describe('PerformanceOptimizedMemoryEngine - Fixed Tests', () => {
  ...
  describe('Core Performance Features', () => {
    it('should track performance metrics for remember operations', async () => {
      ...
      await engine.remember('Test content for performance tracking', 'test-tenant', 'test-agent');
      
      // Check that metrics were recorded
      const metrics = engine.getPerformanceMetrics();
      expect(metrics.recentOperations.length).toBeGreaterThan(0);
      
      const rememberOp = metrics.recentOperations.find(op => op.operationType === 'remember');
    ...
    it('should track performance metrics for recall operations', async () => {
      ...
      engine.clearPerformanceCaches();
      await engine.remember('Content to recall later', 'test-tenant', 'test-agent');
      ...
      const recallOp = metrics.recentOperations.find(op => op.operationType === 'recall');
    ...
    it('should track cache hit rates', async () => {
      ...
      await engine.remember('Unique content for caching test', 'test-tenant', 'test-agent');
    ...
    it('should handle batch operations efficiently', async () => {
      ...
      const requests = [
        { content: 'Batch content 1', tenantId: 'test-tenant', agentId: 'test-agent' },
        { content: 'Batch content 2', tenantId: 'test-tenant', agentId: 'test-agent' },
        { content: 'Batch content 3', tenantId: 'test-tenant', agentId: 'test-agent' }
      ];
    ...
    it('should provide comprehensive cache statistics', async () => {
      ...
      await engine.remember('Cache test content 1', 'test-tenant', 'test-agent');
      await engine.remember('Cache test content 2', 'test-tenant', 'test-agent');
  ...
  describe('Performance Optimization Validation', () => {
    it('should achieve sub-50ms response times for cached operations', async () => {
      // Pre-warm the cache
      await engine.remember('Performance test content', 'test-tenant', 'test-agent');
