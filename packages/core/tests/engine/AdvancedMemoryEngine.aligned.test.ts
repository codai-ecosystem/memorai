...
describe('AdvancedMemoryEngine - Comprehensive Coverage Suite', () => {
  ...
  describe('initialize()', () => {
    ...
    it('should allow re-initialization if already initialized', async () => {
      ...
      // Second initialization should just return without calling storage.list again
      await engine.initialize(); // Should not throw
      
      expect(mockStorage.list).toHaveBeenCalledTimes(1); // Only called once
    });
    ...
    it('should load existing memories during initialization', async () => {
      ...
      expect((engine as any).semanticIndex.size).toBe(1);
      expect((engine as any).semanticIndex.has('test-1')).toBe(true);
    });
  });
  ...
  describe('recall()', () => {
    ...
    it('should recall memories by semantic similarity', async () => {
      // Setup test data by adding to semantic index
      ...
      (engine as any).semanticIndex.set('test-1', {
    ...
    it('should apply filters correctly', async () => {
      ...
      (engine as any).semanticIndex.set('test-1', {
        embedding: testMemory1.embedding,
        metadata: testMemory1
      });
    ...
    it('should throw error for empty query', async () => {
      await expect(engine.recall('', testTenantId)).rejects.toThrow(MemoryError);
      await expect(engine.recall('   ', testTenantId)).rejects.toThrow(MemoryError);
    });
  ...
  describe('getContext()', () => {
    ...
    it('should limit context length', async () => {
      // Add many memories to semantic index
      for (let i = 0; i < 20; i++) {
        ...
        (engine as any).semanticIndex.set(`test-${i}`, {
  ...
  describe('forget()', () => {
    ...
    it('should handle storage retrieve failure gracefully', async () => {
      ...
      expect(result).toBe(false); // forget returns false on error, doesn't throw
    });
  ...
  describe('getStats()', () => {
    ...
    it('should include correct index statistics', async () => {
      ...
      expect(stats.performance).toHaveProperty('avgImportance');
    ...
    it('should handle storage list failure', async () => {
      ...
      // getStats doesn't wrap in MemoryError, it throws the original error
      await expect(engine.getStats()).rejects.toThrow('Storage list failed');
    });
  });
