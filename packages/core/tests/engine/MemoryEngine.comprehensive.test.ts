...
import { MemoryEngine, type RememberOptions, type RecallOptions } from '../../src/engine/MemoryEngine.js';
import { MemoryError, type MemoryMetadata, type MemoryResult } from '../../src/types/index.js';
...
describe('MemoryEngine - Comprehensive Tests', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should handle initialization errors', async () => {
      ...
      await expect(memoryEngine.initialize()).rejects.toThrow('Failed to initialize memory engine: Initialization failed');
    });
    ...
    it('should handle unknown initialization errors', async () => {
      ...
      await expect(memoryEngine.initialize()).rejects.toThrow(MemoryError);
      await expect(memoryEngine.initialize()).rejects.toThrow('Unknown initialization error');
    });
  ...
  describe('Remember functionality', () => {
    ...
    it('should remember with options', async () => {
      ...
      const memoryId = await memoryEngine.remember(content, tenantId, undefined, options);
    ...
    it('should handle empty content', async () => {
      await expect(memoryEngine.remember('', 'tenant-1'))
    ...
    it('should handle embedding errors', async () => {
      ...
      await expect(memoryEngine.remember('test content', 'tenant-1'))
        .rejects.toThrow(MemoryError);
      await expect(memoryEngine.remember('test content', 'tenant-1'))
    ...
    it('should require initialization before remembering', async () => {
      ...
      await expect(uninitializedEngine.remember('test', 'tenant-1'))
        .rejects.toThrow(MemoryError);
    });
  });
  ...
  describe('Recall functionality', () => {
    ...
    it('should recall memories successfully', async () => {
      vi.mocked(mockVectorStore.searchMemories).mockResolvedValue([mockMemoryResult]);

      const results = await memoryEngine.recall('dark mode', 'tenant-1', 'agent-1');
    ...
    it('should recall with custom options', async () => {
      ...
      const results = await memoryEngine.recall('test query', 'tenant-1', undefined, options);
    ...
    it('should handle empty query', async () => {
      await expect(memoryEngine.recall('', 'tenant-1'))
        .rejects.toThrow(MemoryError);
      await expect(memoryEngine.recall('   ', 'tenant-1'))
    ...
    it('should handle embedding errors during recall', async () => {
      ...
      await expect(memoryEngine.recall('test query', 'tenant-1'))
        .rejects.toThrow(MemoryError);
      await expect(memoryEngine.recall('test query', 'tenant-1'))
    ...
    it('should apply time decay when enabled', async () => {
      ...
      const results = await memoryEngine.recall('test query', 'tenant-1', undefined, { time_decay: true });
    ...
    it('should not apply time decay when disabled', async () => {
      ...
      const results = await memoryEngine.recall('test query', 'tenant-1', undefined, { time_decay: false });
  ...
  describe('Forget functionality', () => {
    ...
    it('should forget memories successfully', async () => {
      ...
      const deletedCount = await memoryEngine.forget('outdated information', 'tenant-1', 'agent-1');
    ...
    it('should forget multiple memories', async () => {
      const memory1 = { ...mockMemoryResult, memory: { ...mockMemoryResult.memory, id: 'memory-1' } };
      const memory2 = { ...mockMemoryResult, memory: { ...mockMemoryResult.memory, id: 'memory-2' } };
      
      vi.mocked(mockVectorStore.searchMemories).mockResolvedValue([memory1, memory2]);

      const deletedCount = await memoryEngine.forget('test query', 'tenant-1');

      expect(deletedCount).toBe(2);
      expect(mockVectorStore.deleteMemories).toHaveBeenCalledWith(['memory-1', 'memory-2']);
    });
    ...
    it('should return 0 when no memories match', async () => {
      ...
      vi.mocked(mockVectorStore.searchMemories).mockResolvedValue(memories as any);

      const deletedCount = await memoryEngine.forget('test query', 'tenant-1', 'agent-1', 0.95);
      ...
      expect(deletedCount).toBe(1);
      expect(mockVectorStore.deleteMemories).toHaveBeenCalledWith(['high-score']);
    });
    ...
    it('should handle deletion errors', async () => {
      ...
      await expect(memoryEngine.forget('test query', 'tenant-1'))
        .rejects.toThrow(MemoryError);
      await expect(memoryEngine.forget('test query', 'tenant-1'))
  ...
  describe('Context functionality', () => {
    ...
    it('should get context successfully', async () => {
      ...
      vi.mocked(mockVectorStore.searchMemories).mockResolvedValue(mockContextMemories);
    ...
    it('should handle context errors', async () => {
      ...
      await expect(memoryEngine.context(contextRequest))
        .rejects.toThrow(MemoryError);
      await expect(memoryEngine.context(contextRequest))
        .rejects.toThrow('Failed to get context: Context search failed');
    });
  });
  ...
  describe('Helper methods', () => {
    ...
    it('should calculate importance correctly', () => {
      ...
      expect(engine.calculateImportance('password: secret123')).toBeGreaterThan(0.8);
      expect(engine.calculateImportance('deadline tomorrow')).toBeGreaterThan(0.7);
    ...
    it('should classify memory types correctly', () => {
      ...
      expect(engine.classifyMemoryType('Python is a programming language')).toBe('fact');
  ...
  describe('Error edge cases', () => {
    it('should handle unknown errors gracefully', async () => {
      ...
      await expect(memoryEngine.remember('test', 'tenant-1'))
        .rejects.toThrow('Unknown remember error');
    });

    it('should handle vector store health check failures', async () => {
      await memoryEngine.initialize();
      vi.mocked(mockVectorStore.getHealth).mockRejectedValue(new Error('Health check failed'));      const health = await memoryEngine.getHealth();
  ...
  describe('Edge Cases and Error Conditions', () => {
    beforeEach(async () => {
      ...
      await expect(memoryEngine.recall('', 'tenant1', 'agent1')).rejects.toThrow('Query cannot be empty');
    });

    it('should handle whitespace-only query in recall', async () => {
      await expect(memoryEngine.recall('   \n\t   ', 'tenant1', 'agent1')).rejects.toThrow('Query cannot be empty');
      ...
      const results = await memoryEngine.recall('test query', 'tenant1', 'agent1', { time_decay: true });
      expect(results).toHaveLength(1);
    ...
    it('should handle edge case in importance calculation - casual content', async () => {
      const casualContent = 'The weather is nice today, everything is okay and fine, good times';
      ...
      const memoryId = await memoryEngine.remember(casualContent, 'tenant1', 'agent1');
      expect(memoryId).toBeDefined();
    ...
    it('should handle edge case in importance calculation - long content', async () => {
      const longContent = 'A'.repeat(250) + ' this is important and critical information with a password';
      ...
      const memoryId = await memoryEngine.remember(longContent, 'tenant1', 'agent1');
      expect(memoryId).toBeDefined();
    ...
    it('should ensure importance never exceeds 1.0', async () => {
      const superImportantContent = 'critical urgent important password secret key token deadline remember always never';
      ...
      const memoryId = await memoryEngine.remember(superImportantContent, 'tenant1', 'agent1');
      expect(memoryId).toBeDefined();
    ...
    it('should ensure importance never goes below 0.1', async () => {
      const casualContent = 'weather nice okay fine good ' + 'weather nice okay fine good '.repeat(10);
    ...
    it('should handle personality detection edge cases', async () => {
      const personalityContent = 'This user has a calm personality and good behavior style';
      ...
      const memoryId = await memoryEngine.remember(personalityContent, 'tenant1', 'agent1');
      expect(memoryId).toBeDefined();
    ...
    it('should handle embedding service failures gracefully', async () => {
      vi.mocked(mockEmbeddingService.embed).mockRejectedValueOnce(new Error('Embedding failed'));

      await expect(memoryEngine.remember('test', 'tenant1', 'agent1')).rejects.toThrow('Embedding failed');
    });

    it('should handle vector store failures gracefully', async () => {
      vi.mocked(mockVectorStore.storeMemory).mockRejectedValueOnce(new Error('Vector store failed'));

      await expect(memoryEngine.remember('test', 'tenant1', 'agent1')).rejects.toThrow('Vector store failed');
    });
    ...
    it('should handle forget with no matches', async () => {
      ...
      const deletedCount = await memoryEngine.forget('nonexistent query', 'tenant1', 'agent1');
      expect(deletedCount).toBe(0);
      ...
      vi.mocked(mockVectorStore.searchMemories).mockResolvedValueOnce(memories as any);

      const deletedCount = await memoryEngine.forget('test query', 'tenant1', 'agent1', 0.9);
      ...
      expect(mockVectorStore.deleteMemories).toHaveBeenCalledWith(['mem1', 'mem3']);
    });
  });
  ...
  describe('Memory Type Classification Edge Cases', () => {
    ...
    it('should classify task-related content correctly', async () => {
      const taskContent = 'Please complete the task of reviewing the code by tomorrow';
    ...
    it('should classify emotional content correctly', async () => {
      const emotionalContent = 'I feel happy and excited about this project, love working on it';
    ...
    it('should default to thread type for conversational content', async () => {
      const conversationalContent = 'Let me know what you think about this idea';
