...
describe('StorageAdapter', () => {
  describe('InMemoryStorageAdapter', () => {
    ...
    beforeEach(() => {
      adapter = new InMemoryStorageAdapter();      sampleMemory = {
        id: 'test-123',
    ...
    describe('list', () => {
      beforeEach(async () => {        // Store multiple test memories
        const memories = [
          {
            ...sampleMemory,
            id: 'memory-1',
      ...
      it('should return empty array when no memories match filters', async () => {
        const memories = await adapter.list({ tenantId: 'non-existent-tenant' });
  ...
  describe('PostgreSQLStorageAdapter', () => {
    ...
    beforeEach(() => {
      adapter = new PostgreSQLStorageAdapter('postgresql://localhost:5432/test');
    });

    it('should throw error for store method', async () => {
      const memory = { id: 'test' } as MemoryMetadata;
      await expect(adapter.store(memory)).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });

    it('should throw error for retrieve method', async () => {
      await expect(adapter.retrieve('test-id')).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });

    it('should throw error for update method', async () => {
      await expect(adapter.update('test-id', {})).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });

    it('should throw error for delete method', async () => {
      await expect(adapter.delete('test-id')).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });

    it('should throw error for list method', async () => {
      await expect(adapter.list()).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });

    it('should throw error for clear method', async () => {
      await expect(adapter.clear()).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });

    it('should throw error for clear method with tenantId', async () => {
      await expect(adapter.clear('tenant-1')).rejects.toThrow('PostgreSQL adapter not implemented yet');
    });
  });
  ...
  describe('RedisStorageAdapter', () => {
    ...
    it('should throw error for store method', async () => {
      const memory = { id: 'test' } as MemoryMetadata;
      await expect(adapter.store(memory)).rejects.toThrow('Redis adapter not implemented yet');
    });

    it('should throw error for retrieve method', async () => {
      await expect(adapter.retrieve('test-id')).rejects.toThrow('Redis adapter not implemented yet');
    });

    it('should throw error for update method', async () => {
      await expect(adapter.update('test-id', {})).rejects.toThrow('Redis adapter not implemented yet');
    });

    it('should throw error for delete method', async () => {
      await expect(adapter.delete('test-id')).rejects.toThrow('Redis adapter not implemented yet');
    });

    it('should throw error for list method', async () => {
      await expect(adapter.list()).rejects.toThrow('Redis adapter not implemented yet');
    });

    it('should throw error for clear method', async () => {
      await expect(adapter.clear()).rejects.toThrow('Redis adapter not implemented yet');
    });

    it('should throw error for clear method with tenantId', async () => {
      await expect(adapter.clear('tenant-1')).rejects.toThrow('Redis adapter not implemented yet');
    });
  });
});
