...
import { QdrantVectorStore, MemoryVectorStore, type VectorPoint, type SearchResult } from '../../src/vector/VectorStore.js';
...
describe('VectorStore', () => {
  ...
  const createMockQuery = (overrides: Partial<MemoryQuery> = {}): MemoryQuery => ({
    query: 'test query',
    tenant_id: 'tenant-1',
    limit: 10,
    threshold: 0.7,
  ...
  beforeEach(async () => {
    ...
    qdrantStore = new QdrantVectorStore('http://localhost:6333', 'test-collection', 1536, 'test-key');
  ...
  describe('QdrantVectorStore', () => {
    describe('constructor', () => {
      it('should create instance with API key', () => {
        const store = new QdrantVectorStore('http://localhost:6333', 'test', 1536, 'api-key');
        expect(store).toBeInstanceOf(QdrantVectorStore);
      });

      it('should create instance without API key', () => {
        const store = new QdrantVectorStore('http://localhost:6333', 'test', 1536);
        expect(store).toBeInstanceOf(QdrantVectorStore);
      });
    });
    ...
    describe('initialize', () => {
      ...
      it('should throw VectorStoreError on initialization failure', async () => {
        ...
        await expect(qdrantStore.initialize()).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.initialize()).rejects.toThrow('Failed to initialize Qdrant collection: Connection failed');
      });
      ...
      it('should handle unknown error types', async () => {
        ...
        await expect(qdrantStore.initialize()).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.initialize()).rejects.toThrow('Unknown initialization error');
      });
    });
    ...
    describe('upsert', () => {
      ...
      it('should throw VectorStoreError on upsert failure', async () => {
        ...
        await expect(qdrantStore.upsert(points)).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.upsert(points)).rejects.toThrow('Failed to upsert points: Upsert failed');
      });
      ...
      it('should handle unknown error types in upsert', async () => {
        ...
        await expect(qdrantStore.upsert(points)).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.upsert(points)).rejects.toThrow('Unknown upsert error');
      });
    });

    describe('search', () => {      it('should search with basic query', async () => {
      ...
      it('should throw VectorStoreError on search failure', async () => {
        ...
        await expect(qdrantStore.search([0.1, 0.2], query)).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.search([0.1, 0.2], query)).rejects.toThrow('Search failed: Search failed');
      });
      ...
      it('should handle unknown error types in search', async () => {
        ...
        await expect(qdrantStore.search([0.1, 0.2], query)).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.search([0.1, 0.2], query)).rejects.toThrow('Unknown search error');
      });
    });
    ...
    describe('delete', () => {
      ...
      it('should throw VectorStoreError on delete failure', async () => {
        ...
        await expect(qdrantStore.delete(ids)).rejects.toThrow('Failed to delete points: Delete failed');
      });
      ...
      it('should handle unknown error types in delete', async () => {
        ...
        await expect(qdrantStore.delete(ids)).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.delete(ids)).rejects.toThrow('Unknown delete error');
      });
    });
    ...
    describe('count', () => {
      ...
      it('should throw VectorStoreError on count failure', async () => {
        ...
        await expect(qdrantStore.count('tenant-1')).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.count('tenant-1')).rejects.toThrow('Failed to count points: Count failed');
      });
      ...
      it('should handle unknown error types in count', async () => {
        ...
        await expect(qdrantStore.count('tenant-1')).rejects.toThrow(VectorStoreError);
        await expect(qdrantStore.count('tenant-1')).rejects.toThrow('Unknown count error');
      });
    });
    ...
    describe('healthCheck', () => {
      it('should return true when collection exists', async () => {
        mockQdrantClient.getCollections.mockResolvedValue({
          collections: [{ name: 'test-collection' }, { name: 'other-collection' }]
        });
  ...
  describe('MemoryVectorStore', () => {
    ...
    describe('storeMemories', () => {
      ...
      it('should throw error on memories/embeddings count mismatch', async () => {
        ...
        await expect(memoryVectorStore.storeMemories(mockMemories, embeddings))
    ...
    describe('searchMemories', () => {
      ...
      it('should search memories and transform results', async () => {
        ...
        const embedding = [0.1, 0.2, 0.3];
        const results = await memoryVectorStore.searchMemories(embedding, mockQuery);
        ...
        expect(results[0]).toEqual({
          ...
          score: 0.95,
          relevance_reason: 'Highly relevant to "test search" with excellent semantic match'
        });
      });
      ...
      it('should handle results without TTL', async () => {
        ...
        const results = await memoryVectorStore.searchMemories([0.1, 0.2], mockQuery);
        ...
        expect(results[0].relevance_reason).toBe('Strong relevance to "test search" with good semantic similarity');
      });
    });
    ...
    describe('getHealth', () => {
      ...
      it('should handle health check errors', async () => {
        mockStore.initialize.mockResolvedValue(undefined);
        mockStore.healthCheck.mockRejectedValue(new Error('Health check failed'));
    ...
    describe('generateRelevanceReason', () => {
      it('should generate highly relevant reason for score >= 0.9', async () => {
        ...
        mockStore.search.mockResolvedValue([{
          ...
          score: 0.95,
          payload: { 
            id: 'mem-1', type: 'fact', content: 'test', confidence: 0.9,
            created_at: '2024-01-01T00:00:00.000Z', updated_at: '2024-01-01T00:00:00.000Z',
            last_accessed_at: '2024-01-01T00:00:00.000Z', accessCount: 1, importance: 0.8,
            tags: [], tenant_id: 'tenant-1'
          }
        }]);

        const results = await memoryVectorStore.searchMemories([0.1], { 
          query: 'machine learning', tenant_id: 'tenant-1', limit: 10, threshold: 0.7 
        });

        expect(results[0].relevance_reason).toBe('Highly relevant to "machine learning" with excellent semantic match');
      });
      ...
      it('should generate strong relevance reason for score >= 0.8', async () => {
        ...
        const results = await memoryVectorStore.searchMemories([0.1], { 
          query: 'data science', tenant_id: 'tenant-1', limit: 10, threshold: 0.7 
        });

        expect(results[0].relevance_reason).toBe('Strong relevance to "data science" with good semantic similarity');
      });
      ...
      it('should generate moderate relevance reason for score >= 0.7', async () => {
        ...
        const results = await memoryVectorStore.searchMemories([0.1], { 
          query: 'artificial intelligence', tenant_id: 'tenant-1', limit: 10, threshold: 0.7 
        });

        expect(results[0].relevance_reason).toBe('Moderately relevant to "artificial intelligence" with decent semantic overlap');
      });
      ...
      it('should generate weak relevance reason for score < 0.7', async () => {
        ...
        const results = await memoryVectorStore.searchMemories([0.1], { 
          query: 'neural networks', tenant_id: 'tenant-1', limit: 10, threshold: 0.5 
        });

        expect(results[0].relevance_reason).toBe('Some relevance to "neural networks" but weaker semantic connection');
      });
    });
