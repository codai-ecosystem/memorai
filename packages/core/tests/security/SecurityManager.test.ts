...
describe('InputValidator', () => {
  describe('validate', () => {
    ...
    it('should validate string maxLength', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Field 'name' must be no more than 5 characters");
    });
    ...
    it('should validate array minLength', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Field 'tags' must have at least 2 items");
    });
    ...
    it('should validate array maxLength', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Field 'tags' must have no more than 3 items");
    });
    ...
    it('should validate allowedValues', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Field 'status' must be one of: active, inactive, pending");
    });
    ...
    it('should validate with custom validator returning string', () => {
      const rules: ValidationRule[] = [
        { 
          ...
          type: 'string', 
          required: true, 
          customValidator: (value) => (value as string).length >= 8 ? true : 'Password must be at least 8 characters' 
        }
      ];
  ...
  describe('sanitizeString', () => {    it('should remove HTML tags', () => {
  ...
  describe('validateMemoryContent', () => {
    ...
    it('should reject content exceeding maximum length', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Content exceeds maximum length of 10,000 characters');
    });
    ...
    it('should detect script tags', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Content contains potentially malicious code');
    });
...
describe('RateLimiter', () => {
  ...
  describe('getStatus', () => {    it('should return current status', () => {
      // Make 2 requests
      rateLimiter.isAllowed('tenant1', 'agent1');
      rateLimiter.isAllowed('tenant1', 'agent1');
  ...
  describe('clear', () => {    it('should clear all rate limit data', () => {
      // Make some requests
      rateLimiter.isAllowed('tenant1', 'agent1');
...
describe('SecurityAuditor', () => {
  ...
  describe('getTenantAuditLog', () => {
    ...
    it('should filter by event type', () => {
      const logs = auditor.getTenantAuditLog('tenant1', { eventType: 'access' });
      expect(logs).toHaveLength(1);
  ...
  describe('exportAuditLog', () => {
    ...
    it('should export as CSV', () => {
      ...
      expect(lines[0]).toBe('timestamp,eventType,severity,tenantId,agentId,action,resource,success,errorMessage');
      expect(lines).toHaveLength(2); // Header + 1 data row
      expect(lines[1]).toContain('auth,low,tenant1,agent1,login,auth_system,true,');
    });
    ...
    it('should handle missing optional fields in CSV export', () => {
      ...
      expect(lines[2]).toContain('error,high,tenant2,,failed_action,,false,Something went wrong');
    });
  });
});
...
describe('EncryptionManager', () => {
  ...
  beforeEach(() => {
    encryptionManager = new EncryptionManager('test-encryption-key-with-32-chars');
  });

  describe('constructor', () => {
    it('should create with valid key', () => {
      expect(() => new EncryptionManager('valid-32-character-encryption-key')).not.toThrow();
    });

    it('should throw error for short key', () => {
      expect(() => new EncryptionManager('short')).toThrow('Encryption key must be at least 32 characters long');
    });

    it('should throw error for empty key', () => {
      expect(() => new EncryptionManager('')).toThrow('Encryption key must be at least 32 characters long');
    });
  });
  ...
  describe('encrypt and decrypt', () => {
    ...
    it('should throw error for invalid encrypted format', () => {
      expect(() => encryptionManager.decrypt('invalid')).toThrow('Invalid encrypted text format');
    });
    ...
    it('should throw error for missing IV', () => {
      expect(() => encryptionManager.decrypt(':encrypted')).toThrow('Invalid encrypted text format - missing parts');
    });

    it('should throw error for missing encrypted part', () => {
      expect(() => encryptionManager.decrypt('iv:')).toThrow('Invalid encrypted text format - missing parts');
    });
  });
  ...
  describe('hash and verifyHash', () => {
    ...
    it('should hash data with provided salt', () => {
      ...
      expect(hashed).toBe(`${salt}:${crypto.createHash('sha256').update(data + salt).digest('hex')}`);
    });
...
describe('SecurityManager', () => {
  ...
  describe('constructor', () => {
    ...
    it('should create with minimal config', () => {
      const minimalConfig = { encryptionKey: 'minimal-32-character-key-for-test' };
...
describe('SecurityManager Coverage Enhancement', () => {
  let securityManager: SecurityManager;  beforeEach(() => {
    const config: SecurityManagerConfig = {
      ...
      rateLimiting: {
        maxRequests: 100,
        windowMs: 60000,
        keyGenerator: (tenantId: string, agentId?: string) => `custom:${tenantId}:${agentId || 'default'}` // Target line 249-251
      }
    };
    securityManager = new SecurityManager(config);
...
describe('SecurityManager Coverage Enhancement - Final Push', () => {
    ...
    it('should trigger custom keyGenerator in rate limiter getStatus method (line 250)', () => {
      ...
      const customKeyGen = vi.fn((tenantId: string, agentId?: string) => `custom:${tenantId}:${agentId || 'none'}`);
...
describe('SecurityManager Coverage Enhancement - Lines 154-155', () => {    it('should validate content with empty string (line 154-155)', () => {
      // Test empty content validation to trigger the content.length < 1 check on line 154
      const result = InputValidator.validateMemoryContent('');
      
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Content must be a non-empty string');
    });it('should validate content with whitespace only (line 154-155)', () => {      // Test very short content that should pass but triggers the length check
      const result = InputValidator.validateMemoryContent(' ');
