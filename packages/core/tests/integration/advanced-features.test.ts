...
describe('Advanced Features Integration', () => {
  ...
  describe('Security Integration', () => {
    ...
    it('should validate input data correctly', () => {
      ...
      const rules = [
        { field: 'username', type: 'string' as const, required: true, minLength: 3 },
        { field: 'email', type: 'string' as const, required: true, pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/ },
  ...
  describe('Resilience Integration', () => {
    it('should execute operations with resilience patterns', async () => {
      ...
      const result = await resilienceManager.executeResilient('test-operation', operation, {
        retry: { maxAttempts: 3 }
      });
  ...
  describe('Performance Monitoring Integration', () => {
    it('should track query operations', () => {
      const timer = performanceMonitor.startQuery('remember', 'test-tenant', 'test-agent');
    ...
    it('should track error operations', () => {
      const timer = performanceMonitor.startQuery('recall', 'test-tenant', 'test-agent');
  ...
  describe('Integrated Security and Performance', () => {
    it('should monitor security operations performance', () => {
      ...
      const encryptTimer = performanceMonitor.startQuery('remember', 'security-tenant');
      const encrypted = securityManager.encrypt(data);
      encryptTimer.finish(true, undefined, 1, false);

      // Manually time decryption operation  
      const decryptTimer = performanceMonitor.startQuery('recall', 'security-tenant');
      const decrypted = securityManager.decrypt(encrypted);
  ...
  describe('Integrated Resilience and Security', () => {
    it('should handle security validation with retry logic', async () => {
      ...
      const result = await resilienceManager.executeResilient('secure-validation', secureOperation, {
        retry: { maxAttempts: 3 }
      });
