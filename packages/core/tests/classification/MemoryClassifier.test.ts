...
} from '../../src/classification/MemoryClassifier.js';
...
describe('MemoryClassifier', () => {
  ...
  describe('Edge Cases and Complexity', () => {
    it('should handle mixed content by choosing highest score', () => {
      ...
      const mixedContent =
        'I feel like we should step through this process carefully';
    ...
    it('should handle special characters and formatting', () => {
      const specialContent =
        '🚀 TODO: Implement @user authentication with JWT tokens!!! (deadline: tomorrow) #urgent';
  ...
  describe('Confidence Scoring', () => {
    it('should assign higher confidence to clear matches', () => {
      const clearPersonality =
        'John is a very friendly and outgoing person with a great personality';
      const clearProcedure =
        'Step 1: Initialize the project. Step 2: Install dependencies. Step 3: Configure settings.';
  ...
  describe('Reasoning Quality', () => {
    it('should provide meaningful reasoning for classifications', () => {
      const testCases = [
        {
          text: 'Sarah always behaves professionally',
          expectedType: 'personality',
        },
    ...
    it('should include relevant keywords in reasoning', () => {
      const result = classifier.classify(
        'John has a friendly personality and always behaves nicely'
      );
      expect(result.reasoning.toLowerCase()).toMatch(
        /personality|behavior|friendly/
      );
    });
  });
  ...
  describe('Configuration and Thresholds', () => {
    ...
    it('should validate classification results correctly', () => {
      ...
      invalidResults.forEach(result => {
        expect(
          classifier.validateClassification(result as ClassificationResult)
        ).toBe(false);
      });
    });
  });
  ...
  describe('Pattern Matching', () => {
    it('should match regex patterns correctly', () => {
      const regexTestCases = [
        {
          text: 'User is very friendly',
          pattern: /\b(is|are|seems?|acts?)\s+(very\s+)?\w+ly\b/i,
        },
        { text: 'How to implement authentication', pattern: /\bhow\s+to\b/i },
        {
          text: 'I prefer TypeScript over JavaScript',
          pattern: /\b(prefer|like|dislike|better|worse)\b/i,
        },
        {
          text: 'The server is running',
          pattern: /\b(is|are|was|were)\s+\w+/i,
        },
      ];
  ...
  describe('Multi-keyword Detection', () => {
    it('should detect multiple relevant keywords', () => {
      const multiKeywordText =
        'I prefer this step-by-step approach because it seems better and more reliable';
    ...
    it('should weight important keywords appropriately', () => {
      ...
      const highImportanceText =
        'This is urgent and critical - deadline tomorrow';
