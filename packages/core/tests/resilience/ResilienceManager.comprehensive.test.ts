...
describe("ResilienceManager - Comprehensive Coverage", () => {
  ...
  describe("Circuit Breaker Management", () => {
    ...
    it("should reset circuit breaker that was previously failing", async () => {
      ...
      const failingOperation = async () => {
        callCount++;
        if (callCount <= 6) {
          // Fail more times than max retries to ensure circuit breaker sees failures
      ...
      await manager
        .executeResilient("failing-cb", failingOperation, customOptions)
        .catch(() => {});
      await manager
        .executeResilient("failing-cb", failingOperation, customOptions)
        .catch(() => {});
  ...
  describe("Internal Circuit Breaker Management", () => {
    ...
    it("should handle circuit breaker state transitions", async () => {
      ...
      try {
        await manager.executeResilient(
          "state-transition-cb",
