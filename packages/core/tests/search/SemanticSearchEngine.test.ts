...
import {
  ...
  type SearchContext,
} from '../../src/search/SemanticSearchEngine.js';
...
describe('SemanticSearchEngine', () => {
  ...
  beforeEach(() => {
    ...
    (mockEmbeddingService.embed as any).mockImplementation((text: string) => {
      ...
      const key =
        Object.keys(embeddings).find(k => text.toLowerCase().includes(k)) ||
        'default';
  ...
  describe('Basic Search Functionality', () => {
    ...
    it('should return results sorted by relevance', async () => {
      ...
      for (let i = 0; i < results.length - 1; i++) {
        expect(results[i].searchScore).toBeGreaterThanOrEqual(
          results[i + 1].searchScore
        );
      }
    });
  ...
  describe('Fuzzy Matching', () => {
    ...
    it('should adjust fuzzy threshold correctly', async () => {
      ...
      const highResults = await searchEngine.search(
        'typescrpit',
        mockMemories,
        highThresholdOptions
      );
      const lowResults = await searchEngine.search(
        'typescrpit',
        mockMemories,
        lowThresholdOptions
      );
  ...
  describe('Weight Factors', () => {
    ...
    it('should prioritize semantic similarity with high semantic weight', async () => {
      ...
      const semanticResults = await searchEngine.search(
        'typescript',
        mockMemories,
        semanticWeightedOptions
      );
      const regularResults = await searchEngine.search(
        'typescript',
        mockMemories,
        regularOptions
      );
      ...
      expect(semanticResults[0].searchScore).toBeGreaterThan(
        regularResults[0].searchScore * 0.95
      );
    });
  });
  ...
  describe('Context Integration', () => {
    ...
    it('should boost relevance for context-related memories', async () => {
      ...
      const resultsWithoutContext = await searchEngine.search(
        'project',
        mockMemories
      );
  ...
  describe('Diversity Ranking', () => {
    it('should apply diversity ranking to avoid similar results', async () => {
      ...
      const hasNonTypescriptResult = results.some(
        r => !r.memory.content.toLowerCase().includes('typescript')
      );
  ...
  describe('Performance and Caching', () => {
    it('should cache query embeddings for performance', async () => {
      ...
      const firstCallCount = (mockEmbeddingService.embed as any).mock.calls
        .length;

      // Second search with same query
      await searchEngine.search(query, mockMemories);
      const secondCallCount = (mockEmbeddingService.embed as any).mock.calls
        .length;
  ...
  describe('Error Handling', () => {
    it('should handle embedding service errors gracefully', async () => {
      (mockEmbeddingService.embed as any).mockRejectedValueOnce(
        new Error('Embedding service error')
      );

      await expect(
        searchEngine.search('test query', mockMemories)
    ...
    it('should handle memories without embeddings', async () => {
      const memoriesWithoutEmbeddings = mockMemories.map(m => ({
        ...m,
        embedding: undefined,
      }));
  ...
  describe('SemanticSearchEngine Coverage Enhancement - Final Push', () => {
    ...
    it('should trigger query cache cleanup when exceeding limit (lines 161-165)', async () => {
      ...
      for (let i = 0; i < 1002; i++) {
        promises.push(
          searchEngine.search(`unique query ${i}`, [], { limit: 1 })
        );
      }
    ...
    it('should handle content matching in preference scoring (line 598)', async () => {
      ...
      const memory = createMockMemory(
        'The user likes dark themes and TypeScript',
        'mem-1'
      );
    ...
    it('should handle query cache with multiple embeddings', async () => {
      ...
      (mockEmbeddingService.embed as any).mockResolvedValueOnce([
        0.1, 0.2, 0.3,
      ]);
      (mockEmbeddingService.embed as any).mockResolvedValueOnce([
        0.4, 0.5, 0.6,
