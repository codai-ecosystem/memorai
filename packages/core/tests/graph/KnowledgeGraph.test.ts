...
import {
  ...
  type GraphRelation,
  type GraphQuery,
} from "../../src/graph/KnowledgeGraph.js";
...
describe("KnowledgeGraph", () => {
  ...
  describe("Entity Management", () => {
    ...
    it("should filter entities by properties", async () => {
      ...
      expect(results).toHaveLength(2);
      expect(results.every((e) => e.properties.role === "developer")).toBe(
        true,
      );
    });
    ...
    it("should isolate entities by tenant", async () => {
      ...
      const tenant1Results = knowledgeGraph.findEntities({
        tenant_id: "tenant-1",
      });
      const tenant2Results = knowledgeGraph.findEntities({
        tenant_id: "tenant-2",
      });
  ...
  describe("Relationship Management", () => {
    ...
    beforeEach(async () => {
      personId = await knowledgeGraph.addEntity(
        "John Doe",
        "person",
        {},
        "test-tenant",
      );
      projectId = await knowledgeGraph.addEntity(
        "Project X",
        "project",
        {},
        "test-tenant",
    ...
    it("should get connected entities", async () => {
      const skillId = await knowledgeGraph.addEntity(
        "TypeScript",
        "skill",
        {},
        "test-tenant",
  ...
  describe("Graph Traversal and Paths", () => {
    ...
    beforeEach(async () => {
      ...
      skillId = await knowledgeGraph.addEntity(
        "TypeScript",
        "skill",
        {},
        "test-tenant",
    ...
    it("should handle no path scenarios", async () => {
      const isolatedId = await knowledgeGraph.addEntity(
        "Isolated",
        "test",
        {},
        "test-tenant",
      ...
      const shortestPath = knowledgeGraph.findShortestPath(
        personId,
        isolatedId,
      );
  ...
  describe("Graph Analytics", () => {
    beforeEach(async () => {
      ...
      const person1 = await knowledgeGraph.addEntity(
        "John",
        "person",
        {},
        "test-tenant",
      );
      const person2 = await knowledgeGraph.addEntity(
        "Jane",
        "person",
        {},
        "test-tenant",
      );
      const project1 = await knowledgeGraph.addEntity(
        "Project A",
        "project",
        {},
        "test-tenant",
      );
      const project2 = await knowledgeGraph.addEntity(
        "Project B",
        "project",
        {},
        "test-tenant",
      );
      const skill1 = await knowledgeGraph.addEntity(
        "TypeScript",
        "skill",
        {},
        "test-tenant",
