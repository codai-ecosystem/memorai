...
function fixFileTargeted(filePath) {
    ...
    try {
        ...
        if (content.includes('import ')) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (isImportLine) {
                    ...
                    const isThirdPartyImport = !line.includes('./') && !line.includes('../') && !line.includes('@/') && !isReactImport;
                    const isLocalImport = line.includes('./') || line.includes('../') || line.includes('@/');
        ...
        if (content.includes('ExternalLink') && !content.includes('<ExternalLink')) {
            content = content.replace(/,?\s*ExternalLink\s*,?/g, '');
        }
        if (content.includes('MoreHorizontal') && !content.includes('<MoreHorizontal')) {
        ...
        content = content.replace(
            ...
            (match, whitespace, call) => {
                if (call.includes('Promise') || call.includes('async') || call.includes('await')) {
