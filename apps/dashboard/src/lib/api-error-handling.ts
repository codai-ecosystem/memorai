 ...
 */
function mapErrorToResponse(error: Error): { statusCode: number; errorCode: ErrorCode; message: string; details?: unknown } {
 ...
 */
export function validateHttpMethod(request: NextRequest, allowedMethods: string[]): void {
...
): void {  // Get client identifier (IP address from headers)
...
function validateRateLimit(identifier: string, maxRequests: number, windowMs: number): boolean {
    const now = Date.now();
