...
test.describe('MemorAI Dashboard - Comprehensive Testing', () => {
    ...
    test.describe('Core Functionality', () => {
        test('should load dashboard successfully', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="performance-dashboard"] h1')).toContainText('MemorAI Performance Dashboard');
            ...
            await page.waitForSelector('[data-testid="dashboard-sidebar"]', { timeout: 10000 });

            // Wait for performance metrics to load (we're on performance tab due to beforeEach)
            await page.waitForSelector('[data-testid="performance-total-memories"]', { timeout: 15000 });

            // Check metrics are displayed
            const totalMemories = await page.locator('[data-testid="performance-total-memories"]').textContent();
            ...
            const queryTime = await page.locator('[data-testid="query-time"]').textContent();
        ...
        test('should handle optimization workflow', async ({ page }) => {
            ...
            await page.waitForFunction(() => {
                const button = document.querySelector('[data-testid="optimize-button"]');
    ...
    test.describe('Performance Testing', () => {
        test('should load within performance budget', async ({ page }) => {
            ...
            await Promise.all(
                pages.map(page =>
                    expect(page.locator('h1')).toContainText('MemorAI Performance Dashboard')
                )
            );
            ...
            await expect(page.locator('[data-testid="total-memories"]')).toContainText('2,847');
            await expect(page.locator('[data-testid="memory-usage"]')).toContainText('45.2 GB');
        });
    });
    test.describe('Enterprise Features', () => {
        test('should display enterprise-grade metrics', async ({ page }) => {
            ...
            await page.waitForSelector('[data-testid="cache-performance"]', { timeout: 5000 });
        ...
        test('should show performance insights', async ({ page }) => {
            ...
            await page.getByTestId('nav-overview').click();
            await page.waitForSelector('[data-testid="performance-insights"]', { timeout: 5000 });
    ...
    test.describe('User Experience Flows', () => {
        test('should complete full optimization workflow', async ({ page }) => {
            ...
            await page.waitForSelector('[data-testid="performance-total-memories"]', { timeout: 15000 });
            const initialMemories = await page.locator('[data-testid="performance-total-memories"]').textContent();
            ...
            const finalMemories = await page.locator('[data-testid="performance-total-memories"]').textContent();
            ...
            await page.getByTestId('nav-overview').click();
            await page.waitForSelector('[data-testid="clear-cache-button"]', { timeout: 5000 });
            ...
            await expect(page.locator('[aria-live="polite"]')).toContainText(/cache/i);
        });
    });
});
