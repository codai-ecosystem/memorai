...
test.describe('Individual Component Testing - Every Component', () => {
  ...
  test.describe('Dashboard Header Component', () => {
    test('should display all header elements', async ({ page }) => {
      ...
      const notificationBtn = page.locator(
        'button[aria-label*="notification"], button:has-text("Bell")'
      );
      if ((await notificationBtn.count()) > 0) {
        await notificationBtn.first().click();
      } // Check for settings menu
      const settingsBtn = page.locator(
        'button[aria-label*="setting"], button:has-text("Settings")'
      );
      ...
      const userBtn = page.locator(
        'button[aria-label*="user"], button:has-text("User")'
      );
    ...
    test('should toggle theme if theme switcher exists', async ({ page }) => {
      const themeButtons = page.locator(
        'button[aria-label*="theme"], button:has([data-testid*="theme"])'
      );
  ...
  test.describe('Dashboard Sidebar Component', () => {
    ...
    test('should handle sidebar collapse/expand', async ({ page }) => {
      ...
      const toggleButtons = page.locator(
        'button:has([data-testid*="chevron"]), button:has([data-testid*="toggle"])'
      );
    ...
    test('should highlight active navigation item', async ({ page }) => {
      const navLinks = page.locator(
        '[data-testid="dashboard-sidebar"] a, [data-testid="dashboard-sidebar"] button'
      );
      ...
      if (linkCount > 0) {
        ...
        const activeLink = page.locator(
          '[data-testid="dashboard-sidebar"] .active, [data-testid="dashboard-sidebar"] [aria-current="page"]'
        );
  ...
  test.describe('Memory Overview Component', () => {
    test('should display memory statistics', async ({ page }) => {
      ...
      const statCards = page.locator(
        '[data-testid="memory-overview"] .text-2xl.font-bold'
      );
  ...
  test.describe('Memory Actions Component', () => {
    ...
    test('should open and close memory creation form', async ({ page }) => {
      const addBtn = page
        .locator(
          '[data-testid="quick-action-add-memory"], button:has-text("Add Memory")'
        )
        .first();
      ...
      if ((await addBtn.isVisible()) && (await addBtn.isEnabled())) {
        ...
        const form = page.locator(
          '[data-testid="memory-form"], form[role="form"]'
        );
        try {
          ...
          const closeBtn = page.locator(
            '[data-testid="close-form-button"], button:has-text("Cancel"), button[aria-label*="close"]'
          );
          ...
          console.log(
            'Form did not appear as expected:',
    ...
    test('should validate form input', async ({ page }) => {
      ...
      if (await addBtn.isVisible()) {
        ...
        const submitBtn = page.locator(
          'button[type="submit"]:has-text("Add Memory")'
        );
        if ((await submitBtn.count()) > 0) {
          ...
          if ((await contentField.count()) > 0) {
            const isValid = await contentField.evaluate(
    ...
    test('should handle quick actions', async ({ page }) => {
      ...
      for (const action of quickActions) {
        const actionBtn = page.locator(
          `[data-testid="${action.testid}"], button:has-text("${action.text}")`
        );
    ...
    test('should display search results', async ({ page }) => {
      ...
      if (await resultsComponent.isVisible()) {
        ...
        const resultItems = page.locator(
          '.result-item, [data-testid="result-item"]'
        );
