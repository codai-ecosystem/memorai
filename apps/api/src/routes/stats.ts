...
router.get('/', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        ...
        throw createApiError(`Failed to get statistics: ${errorMessage}`, 500, 'STATS_GET_FAILED');
    }
}));
...
router.get('/agent/:agentId', asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger.error('Failed to get agent statistics', { agentId, error: errorMessage });
        throw createApiError(`Failed to get agent statistics: ${errorMessage}`, 500, 'AGENT_STATS_FAILED');
    }
}));
...
export function updateStats(operation: string, agentId: string, responseTime: number): void {
