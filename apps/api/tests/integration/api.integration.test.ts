...
import { generateTestMemory, generateTestMemories, setTestEnv } from '../helpers/testHelpers';

// Mock memory engine core completely
vi.mock('@codai/memorai-core', () => ({
    UnifiedMemoryEngine: vi.fn().mockImplementation(() => ({
        initialize: vi.fn().mockResolvedValue(undefined),
        remember: vi.fn().mockImplementation(async (agentId: string, content: string, metadata?: any) => ({
        ...
        recall: vi.fn().mockImplementation(async (agentId: string, query: string, limit: number = 10) => {
        ...
        getContext: vi.fn().mockImplementation(async (agentId: string, contextSize: number = 10) => ({
            agentId,
    ...
    describe('Memory Workflow Integration', () => {
        ...
        it('should complete full memory workflow', async () => {
            ...
            const contextResponse = await request(app)
                ...
                .expect(200); expect(contextResponse.body.success).toBe(true);
