...
export const createMockMemoryEngine = (config: Partial<any> = {}) => {
    ...
    return {
        ...
        remember: vi.fn().mockImplementation(async (content: string, tenantId: string, agentId?: string, options?: any) => {
        ...
        recall: vi.fn().mockImplementation(async (query: string, tenantId: string, agentId?: string, options?: any) => {
            const agentMemories = Array.from(memories.values()).filter((m: any) => m.agentId === agentId);
        ...
        getContext: vi.fn().mockImplementation(async (request: any) => {
            ...
            return {
                ...
                memories: agentMemories.slice(0, maxMemories).map((memory: any) => ({ memory, score: 0.9 })),
...
export const generateTestMemories = (count: number, agentId: string = 'test-agent') => {
