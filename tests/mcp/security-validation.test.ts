...
describe('Enterprise Security Manager', () => {
  ...
  describe('Basic Security Validation', () => {
    test('should allow valid memory operations', async () => {
      const result = await securityManager.validateMemoryOperation(validContext);
    ...
    test('should detect SQL injection attempts', async () => {
      ...
      const result = await securityManager.validateMemoryOperation(maliciousContext);
  ...
  describe('Rate Limiting', () => {
    ...
    test('should throttle or block excessive requests', async () => {
      ...
      const blockedCount = results.filter(r => !r.allowed || r.action !== 'allow').length;
  ...
  describe('Integration Validation', () => {
    test('should provide unified security validation response', async () => {
      ...
      expect(['allow', 'throttle', 'block', 'emergency']).toContain(result.action);
    ...
    test('should maintain security context across operations', async () => {
      // First operation
      const result1 = await securityManager.validateMemoryOperation(validContext);
  ...
  describe('Error Handling', () => {
    test('should handle invalid security context gracefully', async () => {
      ...
      const result = await securityManager.validateMemoryOperation(invalidContext);
