...
# Set environment variables for development - DOCKER-FREE MODE
$env:NODE_ENV = "development"
$env:MEMORAI_USE_INMEMORY = "true"
$env:MEMORAI_TIER = "ADVANCED"
$env:MEMORAI_TENANT_ID = "dev-tenant"
# Docker-free mode - external dependencies not required
...
# Skip Docker infrastructure when using in-memory mode
if ($env:MEMORAI_USE_INMEMORY -eq "true") {
    Write-Host "[*] Using in-memory storage - Docker infrastructure not required" -ForegroundColor Yellow
    Write-Host "   [OK] Vector Store: InMemoryVectorStore" -ForegroundColor Green
    Write-Host "   [OK] Storage: InMemoryStorageAdapter" -ForegroundColor Green
    Write-Host "   [OK] Ready for development without external dependencies" -ForegroundColor Green
} else {
